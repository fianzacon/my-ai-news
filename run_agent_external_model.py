...
# --- Database ---
from Database_external_model import init_db, SessionLocal
    ...
    try:
        ...
        recommendation_records = db.execute(text(
            ...
            WITH recent_chats AS (
                ...
                FROM public.chat_history_external
...
async def run_graph_background(run_id: str, topic: str, user_id: str, chat_history: list):
    ...
    try:
        ...
        embedding = result_state.get("embedding")
        db = SessionLocal()
        try:
            db.execute(
                text("INSERT INTO public.chat_history_external (user_id, topic, final_output, embedding) VALUES (:user_id, :topic, :final_output, :embedding)"),
...
async def invoke_agent_start(topic: str = Form(...), user_id: str = Form(...)):
    ...
    try:
        ...
        history_records = db.execute(
            text("SELECT topic, final_output FROM public.chat_history_external WHERE user_id = :user_id ORDER BY created_at DESC LIMIT 5"),
            {'user_id': user_id}
        ).fetchall()
